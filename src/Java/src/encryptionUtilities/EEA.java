...
public class EEA
{
    ...
    public byte[] encryptData(byte[] plainText, String[] keys)
    {
        ...
        byte[] cipherText = new byte[(
            plainText.length + (blockSize - (plainText.length % blockSize)))];
        ...
        for (byte[] key : keysList)
        {
            ...
            for (int x = 0; x < cipherText.length; x++)
            {
                ...
                previousBlock[x % blockSize] = (byte) (keyBlock[x % blockSize]
                                                       ^ byteToXOR);
            }
            loop++;
        }
    ...
    /**
     * Sets which hash algorithm will be used (SHA-256 or SHA-512) based on
     * keyLength
     ...
     * based on how long the user wanted their key to be and uses that as the
     * key and generates as many keys as specified. In cases of 1024 and
     * 2048-bit keys two or four, respectfully, SHA-512 hash keys are generated
     * and merged into one
