...
public class AppUI
{
    ...
    final private String[] bitLenOptions = new String[] { "256", "512", "1024",
                                                          "2048" };
    ...
    private void initialize()
    {
        ...
        encryptDecryptOption.addItemListener(new ItemListener() {
            ...
            {
                if (encryptDecryptOption.getSelectedItem() == "Encrypt"
                    && chckbxGhostMode.isSelected())
                    numKeysPanel.setVisible(true);
                else
                    numKeysPanel.setVisible(false);
            }
        });
        encryptDecryptOption.setModel(new DefaultComboBoxModel<String>(
        ...
        typeToPreformOn.setModel(new DefaultComboBoxModel<String>(
            new String[] { "File", "Directory", "Text" }));
        ...
        chckbxOverwrite.setVisible(true);
        chckbxOverwrite.setToolTipText(
            "The file(s) you encrypt/decrypt will be saved to the same file "
        ...
        chckbxGhostMode.setToolTipText(
            "Ghost mode doesn't utilze a saved key file so the keys used "
        ...
        keyLenSelection.setModel(
            new DefaultComboBoxModel<String>(bitLenOptions));
            ...
            public void actionPerformed(ActionEvent e)
            {
                if (utils.keysExist())
                {
                    int x = JOptionPane.showConfirmDialog(
                        frmEliteEncryption,
                        "You are about to delete your keys file.\n"
                            + "By doing so you will no longer be able to "
                    ...
                    if (x == JOptionPane.YES_OPTION)
                    {
                        if (utils.deleteKeys())
                            JOptionPane.showMessageDialog(
                                frmEliteEncryption,
                                "Your keys file was deleted successfully",
                                "Success", JOptionPane.INFORMATION_MESSAGE);
                        else
                            JOptionPane.showMessageDialog(
                                frmEliteEncryption,
                                "An error occured when trying to delete your "
                                    + "keys file",
                ...
                else
                {
                    if (utils.deleteKeys())
                        ...
                        JOptionPane.showMessageDialog(
                            frmEliteEncryption, "No keys file was deleted",
        ...
        btnCreateKeys.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
                ...
                if (utils.keysExist())
                {
                    int x = JOptionPane.showConfirmDialog(
                        frmEliteEncryption,
                        "You already have a keys file.\n"
                            + "By overwriting it you will no longer be able "
                            + "to decrypt messeages\npreviously encrypted "
                            + "with these keys\n\n"
                            + "Are you sure you would like to continue?",
                        "Caution", JOptionPane.YES_OPTION);
                    if (x == JOptionPane.YES_OPTION)
                ...
                if (createNewKeys)
                {
                        ...
                        try
                        {
                            ...
                            if (tryingToCreateKeys)
                            {
                                ...
                                if (numKeys < 0)
                                {
                                    ...
                                    JOptionPane.showMessageDialog(
                                        frmEliteEncryption,
                                        "The number of keys must be greater "
                                            + "than or equal to 1",
                                        "ERROR", JOptionPane.ERROR_MESSAGE);
                                }
                            }
                    ...
                    if (tryingToCreateKeys)
                    {
                        Object input = JOptionPane.showInputDialog(
                            frmEliteEncryption,
                            "Enter a bit length for your keys",
                            "Create New Keys", JOptionPane.QUESTION_MESSAGE,
                            null, bitLenOptions, "OK");
                        if (input == null)
                            tryingToCreateKeys = false;
                        else
                    ...
                    if (!tryingToCreateKeys)
                    {
                        ...
                        keyLen = 256;
                        JOptionPane.showMessageDialog(
                            frmEliteEncryption, "Key Creation Aborted",
                    ...
                    else
                    {
                        if (utils.writeKeys(eea.createKeys(numKeys, keyLen)))
                            JOptionPane.showMessageDialog(
                                frmEliteEncryption,
                                "Keys were generated and saved successfully",
                                "Success", JOptionPane.INFORMATION_MESSAGE);
                        else
                            JOptionPane.showMessageDialog(
                                frmEliteEncryption,
                                "An error occured trying to save your keys",
                                "ERROR", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
        ...
        scrollPaneKeys.setVerticalScrollBarPolicy(
            JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
    ...
    private void handleGhostModeText()
    {
        if (encryptionOn)
        {
            try
            {
                if (textTextArea.getText().equals(""))
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption,
                        "No text to encrypt was entered.\nPlease enter some "
                            + "text and try again",
                        "ERROR", JOptionPane.ERROR_MESSAGE);
                else
                {
                    int x = JOptionPane.showConfirmDialog(
                        frmEliteEncryption,
                        "You are about to encrypt the text with one time use "
                            + "keys.\nYou will be unable to decypt the text "
                            + "unless\nyou manually copy down the keys after "
                    ...
                    if (x == JOptionPane.YES_OPTION)
                    {
                        ...
                        String encryptedText = new String(
                            eea.encryptData(textTextArea.getText().getBytes(),
                                            keys));
            ...
            catch (Exception e)
            {
                JOptionPane.showMessageDialog(
                    frmEliteEncryption,
                    "An error occured trying to encrypt your text.\n"
                        + "If the problem persists, try saving your text to "
                        + "a .txt file and then try encrypting that",
                    "ERROR", JOptionPane.ERROR_MESSAGE);
            }
        }
        else
        {
            ...
            {
                if (textTextArea.getText().equals("")
                    || keyTextArea.getText().equals(""))
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption,
                        "No text to decrypt and or keys to use were entered."
                ...
                else
                {
                    ...
                    if (eea.validKeyCheck(keys))
                    {
                        ...
                        String decryptedText = new String(
                            eea.decryptData(textTextArea.getText().getBytes(),
                                            keys));
                    ...
                    else
                    {
                        JOptionPane.showMessageDialog(
                            frmEliteEncryption,
                            "One or more keys you entered were invalid",
                            "ERROR", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
            catch (Exception e)
            {
                JOptionPane.showMessageDialog(
                    frmEliteEncryption,
                    "An error occured trying to decrypt your text.\n"
                        + "This is most likely due to a key mismatch, or "
    ...
    private void handleStandardText()
    {
        ...
        if (keys == null)
        {
            JOptionPane.showMessageDialog(frmEliteEncryption,
                                          "No keys were loaded", "ERROR",
        ...
        else
        {
            try
            {
                if (textTextArea.getText().equals(""))
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption,
                        "No text to decrypt was entered.\nPlease enter the "
                            + "encrypted text and try again",
                        "ERROR", JOptionPane.ERROR_MESSAGE);
                else
                {
                    String decryptedText = new String(
    ...
    private String getFilesDirectoriesTextAreaText()
    {
        ...
        if (encryptionOn)
        {
            if (ghostMode)
                infoText +=
                    "WARNING!\nYou are using ghost mode, which means the "
                    + "keys used to encrypt your files will not be saved."
                    + "If you wish to be able to decrypt these files later "
                    + "you must manually copy down the keys once the "
    ...
    private void handleFilesAndDirectories()
    {
        ...
        if (encryptionOn)
        {
            ...
            if (success)
            {
                ...
                if (typeOfOperation == TypeOfOperation.FILE)
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption, "Your file was encrypted",
                        "Success", JOptionPane.INFORMATION_MESSAGE);
                else if (typeOfOperation == TypeOfOperation.DIRECTORY)
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption,
                        "All the files in your directory were encrypted",
                        "Success", JOptionPane.INFORMATION_MESSAGE);
            }
        }
        else
        {
            ...
            if (success)
            {
                ...
                if (typeOfOperation == TypeOfOperation.FILE)
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption, "Your file was decrypted",
                        "Success", JOptionPane.INFORMATION_MESSAGE);
                else if (typeOfOperation == TypeOfOperation.DIRECTORY)
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption,
                        "All the files in your directory were decrypted",
                        "Success", JOptionPane.INFORMATION_MESSAGE);
            }
        }
        if (success)
        ...
        else
        {
            ...
            else
            {
                ...
                if (eea.validKeyCheck(keys))
                {
                    ...
                    if (success)
                    {
                        ...
                        if (typeOfOperation == TypeOfOperation.FILE)
                            ...
                            JOptionPane.showMessageDialog(
                                frmEliteEncryption,
                                "All files in your directory were decrypted",
                                "Success", JOptionPane.INFORMATION_MESSAGE);
                    }
                }
                else
                {
                    JOptionPane.showMessageDialog(
                        frmEliteEncryption,
                        "One or more keys you entered were invalid", "ERROR",
                        JOptionPane.ERROR_MESSAGE);
                    success = false;
                }
            }
    ...
    private void continueToSecondWindow()
    {
        try
        {
            ...
            encryptionOn = encryptDecryptOption.getSelectedItem().equals(
                "Encrypt");
            ...
            keyLen = Integer.parseInt(
                keyLenSelection.getSelectedItem().toString());
                ...
                if (encryptionOn)
                {
                    ...
                    if (typeOfOperation != TypeOfOperation.TEXT)
                    {
                        ...
                        textTextArea.setText(
                            getFilesDirectoriesTextAreaText());
                    }
                }
                else
                {
        ...
        catch (Exception e)
        {
            if (e.getMessage().equals("Invalid File Type"))
            {
                JOptionPane.showMessageDialog(
                    frmEliteEncryption,
                    "Invalid File Type\nYou must select a file with the "
                        + "\'.eea\' file extention",
                    "ERROR", JOptionPane.ERROR_MESSAGE);
                backToFirstWindow();
            }
            ...
            else
            {
                JOptionPane.showMessageDialog(
                    frmEliteEncryption,
                    "Invalid Entry\nThe number of keys cannot be less than 1",
                    "ERROR", JOptionPane.ERROR_MESSAGE);
                numKeys = 3;
            }
        }
    ...
    public String promptForFiles(TypeOfOperation type)
    {
        ...
        if (fc.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)
        {
            ...
            else
            {
                ...
                if (path.substring(path.length() - 4).equals(".eea")
                    || typeOfOperation == TypeOfOperation.DIRECTORY)
                    return path;
                else
                    return "-1";
    ...
    private boolean encryptFile(String[] keys)
    {
        ...
        try
        {
            ...
            {
                File f = new File(uri);
                Files.write(Paths.get(uri + ".eea"),
                            eea.encryptData(fileContents, keys));
                f.delete();
            }
            else
                Files.write(Paths.get(uri + ".eea"),
        ...
        catch (IOException e)
        {
            e.printStackTrace();
            JOptionPane.showMessageDialog(frmEliteEncryption,
                                          "Error reading in the file", "ERROR",
                                          JOptionPane.ERROR_MESSAGE);
            success = false;
        }
        ...
        try
        {
            ...
            if (overwriteFiles)
            {
                Files.write(Paths.get(uri),
                            eea.decryptData(fileContents, keys));
                f.delete();
            }
            else
                Files.write(Paths.get(uri),
        ...
        try
        {
            for (String file : filesList)
            {
                ...
                if (overwriteFiles)
                {
                    ...
                    Files.write(Paths.get(file + ".eea"),
                                eea.encryptData(fileContents, keys));
                    f.delete();
                }
                else
